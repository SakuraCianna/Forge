...
Latest timestamp calendar hardening: strict v0.2 regression and installer smoke reports now reject timezone-qualified timestamps whose date component is not a real calendar date, such as February 31. This prevents JavaScript date normalization from turning impossible dates into accepted evidence.
...
Latest packaging warning recheck: `npm run dist:win` exited 0 on 2026-06-06 and generated `release\Forge-0.2.1-x64-setup.exe`. After moving Vite-bundled renderer and worker packages out of production dependencies, electron-builder's `duplicate dependency references` warning is limited to the mail parser dependency chain, and the installer no longer needs `app.asar.unpacked`. Node still emits `DEP0190`; `NODE_OPTIONS=--trace-deprecation` points to `app-builder-lib`'s node-module collector, so the remaining deprecation warning is tracked as an upstream packaging-stage issue rather than Forge runtime code.
