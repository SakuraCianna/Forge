...
- `writeProjectMemory` redacts common key, token, password, cookie, private-key, and cloud-access-key fragments before writing, and managed `MEMORY.md` entries are redacted again before model injection. Similar memories update the existing entry and merge tags first, so silent writes do not keep appending duplicate notes. Write failures are recorded as current-thread events instead of being treated as successful memory writes. Legacy `.forge/project-memory.json` entries remain readable, and the next write lands in `MEMORY.md`.
