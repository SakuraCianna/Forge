...
- 支持项目隔离的 Agent 记忆。Forge 会在项目扫描时读取根目录 `MEMORY.md`, 并把受控区条目作为可检索的项目记忆注入后续请求; Agent 可通过 `writeProjectMemory` 无感创建或更新其中的受控记忆区, 用户显式要求“记住”的项目规则也会同步写入该文件, 长线程上下文压缩摘要也会在绑定项目时沉淀为 `MEMORY.md` 记忆, 用于长期项目约定、用户纠正和可复用决策。
