...
- `webSearch` 和 `fetchDocs` 当前是 P1 高优先级工具。Agent 需要判断框架、语言、平台 API 或近期行为时, planner 会优先安排官方文档查询; `fetchDocs` 维护常用官方文档映射, 覆盖 React、Next.js、Vue、Vite、TypeScript、Node、Electron、Tailwind、Playwright、Prisma、Supabase、Vercel、OpenAI、Stripe、GitHub Actions、Spring Boot、Maven、Gradle、Java、Python、Go、Rust、Docker 和 Kubernetes 等主题。
...
- 支持项目隔离的 Agent 记忆。Forge 会在项目扫描时读取根目录 `MEMORY.md`, 并把用户手写的普通 Markdown 规则和受控区条目作为可检索的项目记忆注入后续请求; Agent 可通过 `writeProjectMemory` 无感创建或更新其中的受控记忆区, 用户显式要求“记住”的项目规则也会同步写入该文件, 长线程上下文压缩摘要也会在绑定项目时沉淀为 `MEMORY.md` 记忆, 用于长期项目约定、用户纠正和可复用决策。
- `writeProjectMemory` 写入前会脱敏常见 key、token、password、cookie、私钥和云访问密钥片段, `MEMORY.md` 受控区条目注入模型前也会再次脱敏; 相近记忆会优先更新并合并标签, 自动记忆超过上限时会裁剪最旧的非显式条目, 避免无感写入不断追加重复或低信号记录; 写入失败会记录到当前线程事件, 不会假装记忆已经落盘; 旧版 `.forge/project-memory.json` 仍可兼容读取, 下一次写入会落到 `MEMORY.md`。
...
Forge 目前处于 v0.3.x 可用级候选阶段。2026-06-18 已在本仓库用现有正式真实任务回归证据和安装包烟测证据复跑并通过 `npm run quality:v0.3:usable`, 同时覆盖测试、发布检查、Built-in Tools QA 和 Browser QA。核心工作流已经可运行, 包括本地项目索引、Provider 配置、Agent 计划、文件审查、命令执行、Git 操作、Built-in Tools、插件与技能上下文、Extensions 扩展、Agent Profile、记忆、用量统计和本地化错误提示。

v0.3.x 的主要目标不是继续扩张新功能, 而是把 Forge 稳定到可用级本地 AI Coding Agent。当前工具层 QA、lint、typecheck、build、Windows 安装包生成、固定真实任务回归和安装包烟测已有自动化或正式证据入口; 本机长期 `agent-quality-metrics.json` 指标仍需要随真实使用持续采集, 不能把一次固定回归集解释为所有项目场景都无风险。

如果重新生成安装包、更新正式证据文件或改变 Runtime/工具执行链路, 需要重新运行 `npm run quality:v0.3:usable`; 未重新验证前, 不应把新的产物或变更称为已达到可用级。
...
- 更完整的 Runtime 状态机拆分。
- 更细粒度的权限策略。
- 更完整的自动验证和失败恢复闭环, 包括 after-edit hook、失败分类、测试选择和新脚手架 smoke-first 验证策略。
- 更完整的 P1/P2 工具产品化: `fetchDocs` 的文档版本/缓存/引用展示、`webSearch` 的可信源排序、真正的 embedding 语义检索、Browser 工具的预览环境引导, 以及依赖安装、分支/worktree/revert/push 等高风险工具的真实副作用沙箱。
