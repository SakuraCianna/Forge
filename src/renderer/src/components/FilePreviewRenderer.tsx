...
// 渲染文件预览里的 Markdown, 保持文件内容按标准块解析
export function MarkdownPreview({
  ...
  return (
    <MarkdownContent
      compact={compact}
      content={content}
      renderCodeBlock={renderFileCodeBlock}
    />
  );
}

function renderFileCodeBlock(
...
function getCodePreviewLanguage(path: string): ShikiPreviewLanguage {
  ...
  if (specialFileLanguage) {
    return resolveSupportedShikiLanguage(specialFileLanguage);
  }
  ...
  return resolveSupportedShikiLanguage(shikiLanguageByExtension[extension] ?? "text");
}

function resolveSupportedShikiLanguage(language: ShikiPreviewLanguage): ShikiPreviewLanguage {
  if (language === "text" || Object.hasOwn(shikiLanguageRegistry, language)) {
    return language;
  }
...
// 只注册高频工程语言, 未注册的文件仍按纯文本预览, 避免为少见语言生成大量异步 chunk
