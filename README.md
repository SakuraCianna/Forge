...
可用级候选版本需要运行更严格的总门禁。该命令会先执行可用性证据预检并一次列出所有 blocker; 只有真实任务回归证据和安装包人工烟测证据都通过后, 才会继续运行真实任务回归门禁、安装包烟测门禁、完整工程门禁和打包门禁。如果缺少 `docs\V0_2_REGRESSION_RESULTS.json`, 缺少 `docs\V0_2_INSTALLER_SMOKE.json`, 证据报告结构错误、版本不匹配、烟测元数据无效、安装包 SHA-256 不匹配, 或任一门禁未达标, 会以非 0 状态退出:
