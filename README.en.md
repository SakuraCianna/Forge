...
- `webSearch` and `fetchDocs` are now P1 high-priority tools. When framework, language, platform API, or current behavior matters, the planner should schedule official documentation lookup first. `fetchDocs` maintains curated official documentation mappings for common topics including React, Next.js, Vue, Vite, TypeScript, Node, Electron, Tailwind, Playwright, Prisma, Supabase, Vercel, OpenAI, Stripe, GitHub Actions, Spring Boot, Maven, Gradle, Java, Python, Go, Rust, Docker, Kubernetes, common testing/package-manager/database topics, and mobile platforms. `fetchDocs` returns `officialDocs` / `sourceType` / `trustedSource` / `docsCatalogVersion` / `citations` metadata and uses a short TTL in-memory cache inside the current executor; when an official docs fetch fails or a safely searchable topic has no mapping, it returns structured `fallbackSearch` candidates without pretending the search result was already fetched as documentation content, and it does not automatically send untrusted URL-like topics to public search; `webSearch` annotates and promotes official or trusted documentation sources.
...
- Gitee, DingTalk, WeCom, Feishu, Nextcloud, Hetzner Cloud, Salesforce, Zendesk, Intercom, Freshdesk, Pipedrive, Trello, Stripe, Shopify, Mailchimp, Postmark, Twilio, PagerDuty, Datadog, Cloudflare, and Okta currently use manually saved service tokens, API keys, webhooks, or app passwords.
...
- `sendEmail` and chat robot send actions always require explicit confirmation. Forge does not let the agent silently send email or external group messages.
...
- Project-scoped agent memory. Forge reads root `MEMORY.md` during project scans and injects user-authored Markdown rules plus managed entries as searchable project memories for later requests. The agent can use `writeProjectMemory` to silently create or update its managed memory section. Project rules that the user explicitly asks Forge to remember are also mirrored into that file, and long thread context-compaction summaries become `MEMORY.md` memories when the thread is bound to a project, preserving durable conventions, user corrections, and reusable decisions.
- `writeProjectMemory` redacts common key, token, password, cookie, private-key, and cloud-access-key fragments before writing, and managed `MEMORY.md` entries are redacted again before model injection. Similar memories update the existing entry and merge tags first, and automatic memories prune the oldest non-explicit entries after the managed section reaches its limit, so silent writes do not keep appending duplicate or low-signal notes. Write failures are recorded as current-thread events instead of being treated as successful memory writes. Legacy `.forge/project-memory.json` entries remain readable, and the next write lands in `MEMORY.md`.
...
- Pull requests and pushes to any branch first run baseline CI on `windows-latest`: `npm ci`, `npm test`, `npm run typecheck`, `npm run lint`, and `npm run build`; after baseline CI passes, they also run `npm run quality:regression:gate` and `npm run qa:built-in-tools`.
- The v0.3 quality gates prepare the controlled `.tmp-test\quality-gate-sandbox` first and export its resolved absolute path as `FORGE_QA_PROJECT_ROOT`, so Built-in Tools QA does not skip because the project root is missing.
...
- Pushing a `v*` tag or manually running the workflow runs `npm run dist:win` only after baseline CI and the v0.3 quality gates pass, then uploads the `forge-windows-installer` artifact.
...
- More complete P1/P2 tool productization: cross-session persistent caching and fuller citation UI for `fetchDocs`; broader trusted-source coverage for `webSearch`; true embedding-backed semantic search; preview-environment guidance for browser tools; and real side-effect sandboxes for high-risk tools such as dependency installation, branch/worktree/revert/push.
