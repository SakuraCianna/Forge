...
- `changedFiles`: 必须是非空数组, 记录本次任务实际修改过的工作区相对路径。不能使用绝对路径、父级目录跳转或空字符串; 没有这项证据时, 错误文件修改率和无关改动率不能被视为可复盘。
...
- `validations[].afterModification`: 可以省略; 如果显式记录, 必须为 `true`, 因为 v0.2 可用级门禁只接受修改后验证结果。
...
- 格式错误的 run 会被统计为 invalid run, 不会计入有效样本。`invalidRuns[].reasons` 会列出需要修正的字段, 例如 `createdAt`, `complexityForTaskId`, `changedFiles`, `completedInFirstAttemptValidationMismatch`, `failureRecoveredWithoutFailure`, `failureRecoveredMissingAfterFailure`, `validations.command`, `validations.commandForKind`, `validations.exitCode`, `validations.afterModification`, `validations.missingTypecheck`, `validations.missingBuild`, `validations.missingLint`, `validations.duplicateTypecheck`, `validations.duplicateBuild`, `validations.duplicateLint`, `validations.passedExitCodeMismatch`。
- `npm run quality:regression:gate` 会要求 `forgeVersion` 匹配当前 `package.json` 版本, S1-S5、M1-M5、C1-C3 每个固定任务恰好有一条有效结果, 每条 run 包含可审计且不晚于当前时间的执行时间和非空 changedFiles 证据, 每条 run 恰好各记录一次 typecheck、build、lint 验证结果, 每条验证结果包含命令和退出码, 命令必须和验证类型匹配, 且真实任务相关指标达到 usable 阈值; 如果结果文件缺失、版本不匹配、固定任务覆盖不完整、出现未定义任务 ID、重复 taskId、存在 invalid run、指标分母为 0 或指标低于 usable, 命令必须失败。
...
- 验证命令: `rg -n "Forge-0\.2\.0|v0\.2\.0|RELEASE_NOTES_v0\.2\.0" docs/RELEASE.md`
- 一次完成规则: 命令能找到 v0.2.0 示例, 且没有旧版本号残留。
...
- 目标: 核对稳定化计划中的可用级阈值是否和代码定义一致。
- 允许文件: `docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
- 禁止文件: `src/shared/agentQualityMetrics.ts`
- 验证命令: `rg -n "0\.98|0\.02|0\.85|0\.7|0\.45|0\.9|0\.08|0\.1|exactly 0|80%" docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
...
- 目标: 确保 README、发布流程和稳定化计划对 Browser QA 的描述一致。
- 允许文件: `README.md`, `README.en.md`, `docs/RELEASE.md`, `docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
- 禁止文件: `scripts/**`, `src/**`
- 验证命令: `rg -n "qa:built-in-tools:browser|Browser QA|浏览器 QA" README.md README.en.md docs/RELEASE.md docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
...
- 允许文件: `docs/RELEASE.md`, `docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
...
- 目标: 让稳定化计划记录 Built-in Tools QA 覆盖的工具数量、P0 数量和安全断言。
- 允许文件: `docs/superpowers/plans/2026-06-05-v0-2-stabilization.md`
...
- 允许文件: `scripts/run-v0-2-quality-gate.mjs`, `package.json`, `README.md`, `README.en.md`
- 禁止文件: `src/**`
- 验证命令: `npm run quality:v0.2`
