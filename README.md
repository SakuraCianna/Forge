...
- 用户只需在扩展页点击“网页登录授权”, 授权完成后 token 会自动保存到安全存储。OAuth Client ID、Client Secret、同意屏幕和 broker 部署属于产品维护者发布前配置, 不应要求普通用户自行创建。
...
- `FORGE_OAUTH_BROKER_BASE_URL`: 启用 Slack、Notion、Figma、Jira Cloud 和 Discord 的 brokered 授权入口
