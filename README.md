...
- `writeProjectMemory` 写入前会脱敏常见 key、token、password、cookie、私钥和云访问密钥片段, `MEMORY.md` 受控区条目注入模型前也会再次脱敏; 相近记忆会优先更新并合并标签, 避免无感写入不断追加重复条目; 写入失败会记录到当前线程事件, 不会假装记忆已经落盘; 旧版 `.forge/project-memory.json` 仍可兼容读取, 下一次写入会落到 `MEMORY.md`。
