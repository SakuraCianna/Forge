...
- 常规设置里的“输入与运行”可以调整输入框发送快捷键、命令超时、集成终端 Shell、是否自动运行安全步骤、每轮自动运行步数, 以及失败时是否自动生成修复计划, 这些选项都会直接作用到真实执行器; Git Bash 模式会提示 `bash.exe` 是否可启动。
...
- 失败的动作可以重试，也可以基于失败上下文生成修复计划；智能体失败恢复策略会决定是否保持安静、提示用户或自动生成恢复计划。
- 自动失败恢复会记录尝试次数，并在确认队列和动作详情中显示已尝试次数、恢复历史和当前智能体的恢复上限。
