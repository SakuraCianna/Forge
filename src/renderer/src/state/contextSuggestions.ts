...
export function createHeroPromptSuggestions(
...
function createContextPrompts(input: ContextSuggestionInput): string[] {
  ...
  if (input.missingProject) {
    prompts.push(
      isChinese ? "项目路径丢失了吗？" : "Is the project path missing?",
    );
  }
  ...
  if (input.hasRunningThread) {
    prompts.push(
      isChinese ? "继续推进正在运行的任务" : "Continue the running task",
    );
  }

  if (projectName) {
    prompts.push(
      isChinese ? `继续打磨 ${projectName}` : `Keep improving ${projectName}`,
    );
  }
