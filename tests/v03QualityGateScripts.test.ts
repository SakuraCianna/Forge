...
test("v0.3 quality gate script is wired and exposes a safe dry run", async () => {
  const packageJson = JSON.parse(await readFile("package.json", "utf8")) as {
    scripts?: Record<string, string>;
  };

  assert.equal(packageJson.scripts?.["quality:v0.3"], "node scripts/run-v0-3-quality-gate.mjs");

  const scriptSource = await readFile("scripts/run-v0-3-quality-gate.mjs", "utf8");
  ...
  const { stdout } = await execFileAsync(
    process.execPath,
    ["scripts/run-v0-3-quality-gate.mjs"],
    {
      env: {
        ...process.env,
...
test("v0.3 quality gate dry run can skip rebuilding the installer artifact", async () => {
  const { stdout } = await execFileAsync(
    process.execPath,
    ["scripts/run-v0-3-quality-gate.mjs"],
    {
