...
当需要把真实任务回归作为 v0.2.x 可用级证据门时, 使用严格版本。它要求 `forgeVersion` 匹配当前 `package.json` 版本, 固定任务集 S1-S5、M1-M5、C1-C3 每个任务恰好有一条有效结果, 且真实任务相关指标达到 usable 阈值; 文件缺失、报告结构错误、版本不匹配、任务覆盖不完整、出现未定义任务 ID、重复 taskId、存在 invalid run、指标分母为 0 或低于 usable 时都会以非 0 状态退出:
