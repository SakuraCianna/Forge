...
- 必需验证策略会根据变更目标推断验证命令: Maven 后端优先补 `mvn test` 或 `mvn -f Backend/pom.xml test`, Vite/TypeScript 前端优先补对应包管理器 build 命令, Rust 优先补 `cargo test`, Go 模块优先补 `go test ./...`, 文档类改动退回到 `git status --short`。
