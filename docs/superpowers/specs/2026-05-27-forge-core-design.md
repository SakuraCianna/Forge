...
MVP 可以先直接在当前工作区生成 diff, 但设计上应预留 worktree 隔离。
...
1. Forge MVP 是否默认使用当前工作区 diff, 还是第一版就强制使用 Git worktree
2. 是否第一版就加入语音输入
3. 模型设置页是否需要导入 OpenCode 或 Models.dev 风格的 provider catalog
4. 第一版是否允许 Agent 自动运行 install 命令
5. 第一版是否内置中文和英文双语界面
